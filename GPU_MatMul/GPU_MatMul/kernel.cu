...
__global__ void matrixMultiply(float *A, float *B, float *C, int numARows, int numAColumns, int numBRows, int numBColumns)
{
	...
	{
		C[row * numBColumns + col] = 0;
		for (int i = 0; i < numAColumns; i++)
		{
			C[row * numBColumns + col] += A[row * numAColumns + i] * B[i * numBColumns + col];
		}
	}
}
...
int main(int argc, char **argv)
{
	...
	int r = numCRows < 1024 ? numCRows : 1024, c = numCColumns < 1024 ? numCColumns : 1024;
	dim3 blockDim = dim3(r, c);
	dim3 gridDim((numCRows / r) + 1, (numCColumns / c) + 1);
